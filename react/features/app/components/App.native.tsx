...
export class App extends AbstractApp<IProps> {
    ...
    async requestCameraAndMicPermissionsAndroid() {
      try {
   ...
   async  requestCameraAndMicPermissionsIOS(){
    ...
    async componentDidMount() {
        ...
        logger.info(`Loaded SDK ${AppInfo.sdkVersion}${liteTxt}`);
         if (Platform.OS === 'ios') {
              this.requestCameraAndMicPermissionsIOS();
         } else if (Platform.OS === 'android') {
            this.requestCameraAndMicPermissionsAndroid().then(result => {});
        }
    }
    ...
    async _extraInit() {
        ...
        const serverURLChangeEnabled = getState && getFeatureFlag(getState(), SERVER_URL_CHANGE_ENABLED, false);
