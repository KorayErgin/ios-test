 ...
 */
export default class AbstractLobbyScreen<
  P extends IProps = IProps
> extends PureComponent<P, IState> {
  /**
  ...
  constructor(props: P) {
    ...
    this.state = {
      ...
      screenState: props._participantName
        ? SCREEN_STATES.VIEW
        : SCREEN_STATES.EDIT,
    };
  ...
  _getScreenTitleKey() {
    ...
    return !passwordPrompt && this.props._knocking
      ? this.props._isLobbyChatActive
        ? "lobby.lobbyChatStartedTitle"
        : "lobby.joiningTitle"
      : passwordPrompt
      ? "lobby.enterPasswordTitle"
      : "lobby.joinTitle";
  }
  ...
  _onSwitchToKnockMode() {
    this.setState({
      password: "",
      screenState: this.state.displayName
        ? SCREEN_STATES.VIEW
        : SCREEN_STATES.EDIT,
    });
  ...
  _renderContent() {
    ...
    return (
      <>
        ...
        {(screenState === SCREEN_STATES.VIEW ||
          screenState === SCREEN_STATES.EDIT) &&
          this._renderStandardButtons()}
        {screenState === SCREEN_STATES.PASSWORD &&
          this._renderPasswordJoinButtons()}
      </>
    );
  }
...
export function _mapStateToProps(state: IReduxState) {
  ...
  const { isDisplayNameRequiredError, knocking, passwordJoinFailed } =
    state["features/lobby"];
  ...
  const { isLobbyChatActive, lobbyMessageRecipient, messages } =
    state["features/chat"];
  ...
  return {
    ...
    _renderPassword:
      !iAmSipGateway && !disableLobbyPassword && !lobbyWaitingForHost,
